    ...
    if(result[students[i].class] === undefined) {
      if(students[i].score >= 80) {
        result[students[i].class] = {
          pass: [{name: students[i].name, score: students[i].score}],
          fail: []
        }
      } else {
        result[students[i].class] = {
          pass: [],
          fail: [{name: students[i].name, score: students[i].score}]
        }
      }
      } else {
        if(students[i].score >= 80) {
          let temp = {name: students[i].name, score: students[i].score}
          result[students[i].class].pass.push(temp)
        } else {
          let temp = {name: students[i].name, score: students[i].score}
          result[students[i].class].fail.push(temp)
        }
      }
    }
    return result
